...
class ClassSourceManipulator
{
    ...
    protected function buildProperty(
        ...
        $newPropertyBuilder = $this->createPropertyBuilder($name);
        ...
        return $newPropertyBuilder->getNode();
    }
    ...
    protected function addStatementToConstructor(Node\Stmt $stmt): void
    {
        if (!$this->getConstructorNode()) {
    ...
    public function getPropertyNode(string $propertyName): ?Node\Stmt\Property
    {
        return $this->getClassNode()->getProperty($propertyName);
    }
    ...
    protected function createDocBlock(array $commentLines): string
    {
        $docBlock = "/**\n";
    ...
    private function addMethod(Node\Stmt\ClassMethod $methodNode): void
    {
        ...
        $existingIndex = null;
        if ($this->methodExists($methodName->name)) {
            if (!$this->overwrite) {
                return;
            }
     ...
     */
    protected function addNodeAfterProperties(Node $newNode, bool $addNewline = true): void
    {
        $classNode = $this->getClassNode();
    ...
    protected function propertyExists(string $propertyName): bool
    {
        foreach ($this->getClassNode()->stmts as $i => $node) {
